function [P, Pstring] = stsoporte (wi, betaB, wip, ct, N, betaBr, phiD, phiB, i)
% Description:
% Calculate the support force in the block toppling failure allows balance
% in the acting forces and assure the system equilibrium before this failure.
%
% Main Reference:
...
fi0(1) = ((wi(1) * sin(betaBRad)) - ((wip(1) + ((0)-1)*(0)) * ...
    ((cos(betaBrRad)) + (tan(phiD)*sin(betaBrRad))))) / ...
    (((wi(1) * cos(betaBRad)) - (((wip(1)) + ((0) - 1) * (0)) * ...
    ((tan(phiDRad)) *(cos(betaBrRad))) - (sin(betaBrRad))) )* (tan(phiBRad))); 

for i = 2:numVals
    fi1(i) = ((wi(i) * sin(betaBRad)) - ((wip(i) + ((ct(i-1))-1)*(N(i-1))) * ...
    ((cos(betaBrRad)) + (tan(phiD)*sin(betaBrRad))))) / ...
    (((wi(i) * cos(betaBRad)) - (((wip(i)) + ((ct(i-1)) -1) * (N(i-1))) * ...
    ((tan(phiDRad)) * (cos(betaBrRad))) - (sin(betaBrRad))) ) * (tan(phiBRad)));
end
...
for j = 1:numVals
    find(fi < 1);
    if fi < 1
       %Pstring = (['El Talud es estable al volcamiento, no se requiere anclaje, ', ...
       %'no hay deslizamiento de bloques, P = 0 kN.']);
       Pstring = (['The slope is stable for topplig, ', ...
       'it is not necessary anchor, ', ...
       'there is no slip of the blocks, P = 0 kN.']);
       P = 0;
    end
    for i = find(fi>=1, 1):1:find(fi>=1, 1)
       P = ((N(i-1)) * (cos(betaBrRad)+ (sin (betaBrRad)*(tan(phiBRad))) + ...
       (sin(betaBrRad)*tan(phiDRad)) - ...
       (cos(betaBrRad)*tan(phiDRad)*tan(phiBRad)) )) + ...
       (sum(WT))*((sin(betaBRad)) - ((cos(betaBRad))*tan(phiBRad)));
       Pstring = '';
    end
end
%display (P);
